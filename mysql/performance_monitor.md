...
监控MySQL的前提是要知道需要监控那些性能指标，通过MySQL的`SHOW status`可以查阅到所有MySQL运行的状态信息。通过查阅指定的状态值可以分析出：

    * MySQL过去一段时间或者当前运行的状态
    * 因硬件升级或者系统配置的改变而诊断对Mysql数据库性能影响
    * 在Mysql数据库系统出现故障要能够及时收到告警
    * 为日后编写运维报告提供各项数据指标供分析
