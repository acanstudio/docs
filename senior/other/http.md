...
HTTP（超文本传输协议）是一个属于应用层的面向对象的协议。基于请求与响应模式的、无状态的、应用层的协议，常基于TCP的连接方式，HTTP1.1版本中给出一种持续连接的机制，绝大多数的Web开发，都是构建在HTTP协议之上的Web应用。由于其简捷、快速的方式，适用于分布式超媒体信息系统。它于1990年提出，经过几年的使用与发展，得到不断地完善和扩展。目前在WWW中使用的是HTTP/1.0的第六版，HTTP/1.1的规范化工作正在进行之中，而且HTTP-NG(Next Generation of HTTP)的建议已经提出。
...
http表示要通过HTTP协议来定位网络资源；host表示合法的Internet主机域名或者IP地址；port指定一个端口号，为空则使用缺省端口80；abs_path指定请求资源的URI；如果URL中没有给出abs_path，那么当它作为请求URI时，必须以“/”的形式给出，通常这个工作浏览器自动帮我们完成。如： www.guet.edu.cn（浏览器自动转换成：http://www.guet.edu.cn/）；2、http:192.168.0.116:8080/index.jsp 

HTTP协议之请求篇
--------------------

http请求由三部分组成，分别是：请求行、消息报头、请求正文，这里先重点介绍一下请求行，消息报头和请求正文在下面的相关内容里再做介绍。
...
POST方法：要求被请求服务器接受附在请求后面的数据，常用于提交表单。如：
...
状态行格式如下：HTTP-Version Status-Code Reason-Phrase CRLF；其中，HTTP-Version表示服务器HTTP协议的版本；Status-Code表示服务器发回的响应状态代码；Reason-Phrase表示状态代码的文本描述，如：HTTP/1.1 200 OK （CRLF）。
...
常见状态代码、状态描述、说明：
    ...
    * 404 Not Found //请求资源不存在，例如在输入了错误的URL时，就会返回404状态
...
Http指纹识别的原理大致上也是相同的：记录不同服务器对Http协议执行中的微小差别进行识别。Http指纹识别比TCP/IP堆栈指纹识别复杂许多，理由是定制Http服务器的配置文件、增加插件或组件使得更改Http的响应信息变的很容易，这样使得识别变的困难；然而定制TCP/IP堆栈的行为需要对核心层进行修改，所以就容易识别。Http指纹识别工具Httprint，它通过运用统计学原理，组合模糊的逻辑学技术，能很有效的确定Http服务器的类型。它可以被用来收集和分析不同Http服务器产生的签名。
