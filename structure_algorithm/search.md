...
前面几种高效的查找方法是在有序的基础上查找的，但有些数据集增长非常快，没办法针对关键字排序，这种情况下，可以通过索引来优化查找。索引就是把一个关键字与它对应的记录相关联的过程，一个索引由若干个索引项组成，每个索引项至少包含关键字和其对应的记录在存储器中的位置等信息。索引技术是组织大型数据库一级磁盘文件的一种重要技术。

索引可分为线性索引、树形索引和多级索引。这里只介绍线性索引，线性索引就是索引项集合组织为线性结构，也称索引表，常用的有：

    * 稠密索引，在线性索引中，将数据集中的每个记录对应一个索引项。索引项一定是按照关键字有序的排列；适合数据量不是非常大的情况；
    * 分块索引，分块有序，是把数据集的记录分成若干块，并且这些块需要满足块内无序和块间有序；每块对应一个索引项；索引项包括，最大关键码、记录个数一级指向块首数据元素的指针；分块索引的查找，在分块索引表中查找关键字所在的块，根据块首指针找到相应的块，并在块中顺序查找关键码；
    * 倒排索引（inverted index），索引项的结构包括次关键码和记录号表；记录号表存储具有相同次关键字的所有记录的记录号（指向记录的指针或记录的主关键字）；是由属性值确定记录的位置，而不是由记录来确定属性值；
